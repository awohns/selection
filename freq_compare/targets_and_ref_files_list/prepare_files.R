...
thousandg_list <- list.files(path="/ebc_data/awwohns/selection/getting_1000g_refs/withrsids/joined", full.names=TRUE) 
file_list <- list()

#print(thousandg_list[grep(paste0("chr",1,"\\."),thousandg_list)])
for (i in 1:22) {
	#file_list[[i]] <- target_list[grep(paste0("/",i,"_"),target_list)]
	file_list[[i]] <- cbind(target_list[grep(paste0("/",i,"_"),target_list)], thousandg_list[grep(paste0("chr",i,"j"),thousandg_list)])
