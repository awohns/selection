...
#echo(x)
#print(args[1])
#print(substr(args[1], 1, 5))
#no_col <- max(count.fields(args[1], sep = "\t"))
#thousandgenomeref <- read.table(args[1], sep="\t", fill=TRUE, row.names = NULL, col.names=1:no_col)

#reference <- read.table(args[1], sep="\t", fill=TRUE)
#targetfreq <- read.table(args[2], sep="\t")
